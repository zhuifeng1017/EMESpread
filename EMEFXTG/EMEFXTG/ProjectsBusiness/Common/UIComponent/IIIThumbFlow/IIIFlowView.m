...
#import "SDImageCacheOld+IIIThumb.h"
...
- (void)prepareToReload {
    ...
    _cellCount = (int)[self.flowDelegate numberOfCells];
    _columnCount = (int)[self.flowDelegate numberOfColumns];
    if (!_columnCount) {
        _columnCount = 1;
    }
...
- (void)_reloadData {
    ...
    //        _columnCount != [self.flowDelegate numberOfColumns]) {//DRAFT, hchai I want refreah all cell when data content changed
...
- (void)checkCellsFrom:(int)si direction:(int)dir endHeight:(CGFloat)eh {
    ...
    for (int i = si; i < _cellCount && i >= 0; i += dir) {
        ...
        if (c) {
            ...
            [self addSubview:v];
            [self.flowDelegate updateFlowView:self forCell:c atIndex:i];
        } else {
            // Load cell
...
- (BOOL)loadCell:(IIIFlowCell *)cell AtIndex:(int)index {
    ...
    if (!thumb) {
        ...
        oImg = [[SDImageCacheOld sharedThumbImageCache] imageFromKey:url];
        if (!oImg) {
            ...
            if (d.local_url.length) {
                ...
                if (!oImg) {
                    ...
                    [[SDImageCacheOld sharedThumbImageCache] storeImage:oImg forKey:url];
                }
            } else {
                // If it's a web url, asynchronize download image, suspend loading cells
                if (![_downloadingURLs objectForKey:d.web_url]) {
                    [SDWebImageDownloaderOld downloaderWithURL:[NSURL URLWithString:d.web_url] delegate:self userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:index] forKey:INDEX_KEY] lowPriority:YES];
...
#pragma mark - SDWebImageDownloaderDelegateOld methods
- (void)imageDownloader:(SDWebImageDownloaderOld *)downloader didFinishWithImage:(UIImage *)image {
    ...
    if ([[NSString stringWithFormat:@"%i", index] isEqualToString:[_downloadingURLs objectForKey:urlStr]]) {
        ...
        [[SDImageCacheOld sharedThumbImageCache] storeImage:image forKey:urlStr];
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (!decelerate) {
        ...
        if ((abs(vb - _l_bottom) < SAFE_SCROLL_LENGTH) || ((abs(vt - _l_top) < SAFE_SCROLL_LENGTH) && _l_top > 0)) {
            [self reloadData];
        }
    }
}

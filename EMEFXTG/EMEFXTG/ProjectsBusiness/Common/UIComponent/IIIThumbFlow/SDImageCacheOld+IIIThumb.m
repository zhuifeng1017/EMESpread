...
static SDImageCacheOld *thumbInstance;
static NSInteger thumbMaxAge = 30 * 24 * 60 * 60; // one month
static NSString *thumbSubDirectory = @"iii_thumb";

@implementation SDImageCacheOld (IIIThumb)

#pragma mark SDImageCacheOld (class methods)

+ (SDImageCacheOld *)sharedThumbImageCache {
    if (thumbInstance == nil) {
        ...
        thumbInstance = [[SDImageCacheOld alloc] initWithBasePath:basePath cacheAge:thumbMaxAge];
        [thumbInstance cleanDisk];
    }
...
- (id)initWithBasePath:(NSString *)path cacheAge:(NSInteger)age {
    if ((self = [super init])) {
        [SDImageCacheOld setMaxCacheAge:age];

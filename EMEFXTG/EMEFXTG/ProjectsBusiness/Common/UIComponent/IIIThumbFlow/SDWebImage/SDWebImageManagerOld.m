...
#import "SDWebImageManagerOld.h"
#import "SDImageCacheOld.h"
#import "SDWebImageDownloaderOld.h"
#import <objc/message.h>

static SDWebImageManagerOld *instance;

@implementation SDWebImageManagerOld
...
+ (id)sharedManager {
    if (instance == nil) {
        instance = [[SDWebImageManagerOld alloc] init];
    }
...
- (UIImage *)imageWithURL:(NSURL *)url {
    return [[SDImageCacheOld sharedImageCache] imageFromKey:[self cacheKeyForURL:url]];
}

/*
 * @deprecated
 */
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegateOld>)delegate retryFailed:(BOOL)retryFailed {
 ...
 * @deprecated
 */
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegateOld>)delegate retryFailed:(BOOL)retryFailed lowPriority:(BOOL)lowPriority {
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegateOld>)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo {
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:
                                           ...
                                           nil];
    [[SDImageCacheOld sharedImageCache] queryDiskCacheForKey:[self cacheKeyForURL:url] delegate:self userInfo:info];
}
...
- (void)cancelForDelegate:(id<SDWebImageManagerDelegateOld>)delegate {
    ...
    while ((idx = [downloadDelegates indexOfObjectIdenticalTo:delegate]) != NSNotFound) {
        SDWebImageDownloaderOld *downloader = SDWIReturnRetained([downloaders objectAtIndex:idx]);
...
- (void)cancelAll {
    for (SDWebImageDownloaderOld *downloader in downloaders) {
        [downloader cancel];
    }
...
- (NSUInteger)indexOfDelegate:(id<SDWebImageManagerDelegateOld>)delegate waitingForURL:(NSURL *)url {
...
- (void)imageCache:(SDImageCacheOld *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info {
    ...
    id<SDWebImageManagerDelegateOld> delegate = [info objectForKey:@"delegate"];
...
- (void)imageCache:(SDImageCacheOld *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info {
    ...
    SDWebImageDownloaderOld *downloader = [downloaderForURL objectForKey:url];

    if (!downloader) {
        downloader = [SDWebImageDownloaderOld downloaderWithURL:url delegate:self userInfo:info lowPriority:(options & SDWebImageLowPriority)];
...
#pragma mark SDWebImageDownloaderDelegateOld
...
- (void)imageDownloader:(SDWebImageDownloaderOld *)downloader didUpdatePartialImage:(UIImage *)image {
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--) {
        ...
        SDWebImageDownloaderOld *aDownloader = [downloaders objectAtIndex:uidx];
        if (aDownloader == downloader) {
            id<SDWebImageManagerDelegateOld> delegate = [downloadDelegates objectAtIndex:uidx];
            SDWIRetain(delegate);
...
- (void)imageDownloader:(SDWebImageDownloaderOld *)downloader didFinishWithImage:(UIImage *)image {
    ...
    if (image) {
        ...
        [[SDImageCacheOld sharedImageCache] storeImage:image

...
#import "SDWebImageDownloaderOld.h"
#import "SDWebImageDecoderOld.h"
#import <ImageIO/ImageIO.h>

@interface SDWebImageDownloaderOld (ImageDecoder) <SDWebImageDecoderDelegate>
@end

NSString *const SDWebImageDownloadStartNotificationOld = @"SDWebImageDownloadStartNotificationOld";
NSString *const SDWebImageDownloadStopNotificationOld = @"SDWebImageDownloadStopNotificationOld";

@interface SDWebImageDownloaderOld ()
@property (nonatomic, retain) NSURLConnection *connection;
@end

@implementation SDWebImageDownloaderOld
...
#pragma clang diagnostic pop
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:activityIndicator name:SDWebImageDownloadStartNotificationOld object:nil];
        [[NSNotificationCenter defaultCenter] removeObserver:activityIndicator name:SDWebImageDownloadStopNotificationOld object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"startActivity")
                                                     name:SDWebImageDownloadStartNotificationOld
                                                   object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"stopActivity")
                                                     name:SDWebImageDownloadStopNotificationOld
                                                   object:nil];
    }

    SDWebImageDownloaderOld *downloader = SDWIReturnAutoreleased([[SDWebImageDownloaderOld alloc] init]);
    downloader.url = url;
...
- (void)start {
    // In order to prevent from potential duplicate caching (NSURLCache + SDImageCacheOld) we disable the cache for image requests
    ...
    if (connection) {
        [[NSNotificationCenter defaultCenter] postNotificationName:SDWebImageDownloadStartNotificationOld object:self];
    } else {
...
- (void)cancel {
    if (connection) {
        [connection cancel];
        self.connection = nil;
        [[NSNotificationCenter defaultCenter] postNotificationName:SDWebImageDownloadStopNotificationOld object:self];
    }
}
...
- (void)connection:(NSURLConnection *)aConnection didReceiveResponse:(NSURLResponse *)response {
    if (![response respondsToSelector:@selector(statusCode)] || [((NSHTTPURLResponse *)response)statusCode] < 400) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:SDWebImageDownloadStopNotificationOld object:nil];
    ...
    if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)]) {
        ...
        if (width + height > 0 && totalSize < expectedSize) {
            ...
            if (partialImageRef) {
                ...
                [[SDWebImageDecoderOld sharedImageDecoder] decodeImage:image
                                                          withDelegate:self
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection {
    ...
    if ([delegate respondsToSelector:@selector(imageDownloader:didFinishWithImage:)]) {
        ...
        [[SDWebImageDecoderOld sharedImageDecoder] decodeImage:image withDelegate:self userInfo:nil];
    }
}
...
- (void)imageDecoder:(SDWebImageDecoderOld *)decoder didFinishDecodingImage:(UIImage *)image userInfo:(NSDictionary *)aUserInfo {
